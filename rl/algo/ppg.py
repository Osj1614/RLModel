...
class PPG:
    def __init__(self, device, model, name='', nenvs=32, nsteps=256, train_steps=200e6, lr=2.5e-4, minibatches=8, critic_coef=1, ent_coef=0.01, pred_coef=0.0,\
         gamma=0.99, epsilon=0.2, lamda=0.95, policy_epochs=1
         , aux_interval=16, aux_epochs=6, clone_coef=0.5):
        self.model = model.to(device)
        self.optimizer_policy = AdamW(model.parameters(), lr=lr, weight_decay=1e-7)
        self.optimizer_aux = AdamW(model.parameters(), lr=lr, weight_decay=1e-7)
        self.name = name
        self.device = device
        ...
        self.clone_coef = clone_coef
        self.aux_batches = minibatches * aux_interval
        ...
        self.aux_losses = ['Clone_loss', 'Aux_critic_loss', 'Aux_loss'] #, 'Prediction_loss'
    ...
    def run_trains(self, s_lst, a_lst, returns_lst, advantage_lst, action_prob_lst):
        ...
        self.update_cnt += 1
        if self.update_cnt >= self.aux_interval:
            self.update_cnt = 0
            loss_sum = np.zeros(len(self.aux_losses))
            
            self.obs_lst = th.cat(self.obs_lst, dim=0)
            self.ret_lst = th.cat(self.ret_lst, dim=0)
            #self.gt_lst = th.cat(self.gt_lst, dim=0)

            size = self.update_steps * self.aux_interval
            minibatch_size = size // self.aux_batches
            order = np.arange(size)
            old_policy_lst = []
            for i in range(0, size, minibatch_size):
                end = i+minibatch_size
                _, policy = self.model(self.obs_lst[i:end].to(self.device))
                old_policy_lst.append(policy.detach().cpu())
            old_policy_lst = th.cat(old_policy_lst, dim=0)
            ...
            for _ in range(self.aux_epochs):
                ...
                for i in range(0, size, minibatch_size):
                    ...
                    ind = order[i:end]
                    slices = (arr[ind].to(self.device) for arr in (self.obs_lst, self.ret_lst, old_policy_lst)) # , self.gt_lst
                    loss_sum += self.train_aux_phase(*slices)

            self.obs_lst = []
            self.ret_lst = []
            #self.gt_lst = []
            loss_sum /= self.aux_batches * self.aux_epochs
            for i in range(len(self.aux_losses)):
                self.write_log(self.aux_losses[i], loss_sum[i])

    def train_policy_phase(self, states, actions, returns, advantages, action_probs):
        value, policy = self.model(states, detach_value=True)
    ...
    def train_aux_phase(self, states, returns, old_policy): #, gts
        value, policy = self.model(states)
        policy_dist = self.model.get_dist(policy, detach_std=True)
        critic_loss = F.mse_loss(value, returns)
        
        #pred_loss = F.mse_loss(pred, gts)

        clone_loss = distributions.kl_divergence(self.model.get_dist(old_policy), policy_dist).sum(axis=-1).mean()
        
        if th.isinf(clone_loss).any():
            print('WARNING: clone loss has inf')
            #print('probs are')
            #print(policy[th.where(policy_dist.probs==0)[:-1]])
            clone_loss = th.zeros(1).to(self.device)
        
        aux_loss = critic_loss * self.critic_coef + clone_loss * self.clone_coef #pred_loss * self.pred_coef + 
        ...
        return np.array([clone_loss.item(), critic_loss.item(), aux_loss.item()]) #, pred_loss.item()
    ...
    def train_batches(self, batches):
        ...
        self.obs_lst.append(s_lsts)
        #self.gt_lst.append(target_lsts)
        self.ret_lst.append(returns_lsts)
